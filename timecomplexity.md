Time Complexity Cheat Sheet — Simple Version
...
Any loops at all? No → O(1). Yes → go to 2.

Loops nested? No, they're separate one after another → add the costs, usually O(n). Yes → go to 3.

Does the inner loop run fully every single outer iteration? Yes → multiply, usually O(n²). No, it's gated by a condition → add up all iterations across the whole run instead of multiplying, usually O(n).

Any hidden O(n) operation inside a loop (reverse, sum, sort, slice)? Yes → multiply that into your loop cost too.
